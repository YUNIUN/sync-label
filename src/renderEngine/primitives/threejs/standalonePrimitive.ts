...
export class StandalonePrimitive {
    ...
    private init(): void {
        ...
        geometry.setAttribute('instanceShown', new InstancedBufferAttribute(new Float32Array(MAX_COUNT * 3), 3));
        // { width, height }
        geometry.setAttribute('instanceSize', new InstancedBufferAttribute(new Float32Array(MAX_COUNT * 2), 2));
        // { minWidth, minHeight, maxWidth, maxHeight }
        geometry.setAttribute('instanceLimit', new InstancedBufferAttribute(new Float32Array(MAX_COUNT * 4), 4));
        // { r, g, b }
        geometry.setAttribute('instanceColor', new InstancedBufferAttribute(new Float32Array(MAX_COUNT * 3), 3));
    ...
    private updateByIndex(index: number, data: T_StandaloneAnnotate): void {
        ...
        object3D.rotation.copy(new Euler(validData.rotation.x, validData.rotation.y, validData.rotation.z));
        ...
        instanceLimit.setXYZW(index, validData.minWidth ?? 0, validData.minHeight ?? 0, validData.maxWidth ?? 0xffffff, validData.maxHeight ?? 0xffffff);
        // 2. 显示元素
