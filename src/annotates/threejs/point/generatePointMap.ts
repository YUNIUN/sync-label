...
import { T_IDType, T_StandaloneAnnotate, T_StandaloneDrawData } from "../../../types/renderEngine/renderEngine";
    ...
    effect((data: T_StandaloneDrawData) => {
        ...
        if (data) {
            ...
            while (data.append.size || data.modify.size || data.remove.size) {
                ...
                if (!pointPrimitive) {
                    pointPrimitive = engine.generateStandalonePrimitive(geometry, material, RenderOrderMap["point"]);
